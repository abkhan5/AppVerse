...
internal class AzureEntityManager 
{
    ...
    //private readonly string allowedCharacters = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstovwxyz0123456789-".ToCharArray();
    ...
    public async Task CreateTopicSubscription(string topicName, string subscriptionName, string forwadingQueueName, IDictionary<string, string> custumRules, CancellationToken cancellationToken)
    {
        var client = GetClient();
        ...
        if (!string.IsNullOrEmpty(forwadingQueueName))
            subscriptionOptions.ForwardTo = forwadingQueueName;
        ...
        if (custumRules != null && custumRules.Count > 0)
            await CreateRules(topicName, subscriptionName, custumRules, cancellationToken);
    }
