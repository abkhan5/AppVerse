...
internal class AzureServiceSenderBus : IMessageSender
{
    private const string scheduledMessageKey = "sc";
    private readonly AzureEntityManager entityManager;
    private readonly ILogger<AzureServiceSenderBus> logger;
    ...
    private async Task SaveScheduleMessage(List<ScheduledMessageRecords> scheduledRecords, CancellationToken cancellationToken) =>
        await distributedCacheRepository.SetAsync(scheduledMessageKey, scheduledRecords, new TimeSpan(365, 0, 0, 0), cancellationToken);
    ...
    public async Task SendMessage<T>(T message, CancellationToken cancellationToken) =>
        await SendMessage(message, message.ToString(), cancellationToken);
