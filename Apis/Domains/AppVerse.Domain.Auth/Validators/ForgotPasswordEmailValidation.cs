...
public class ForgotPasswordEmailValidation : AbstractValidator<ForgotPasswordEmail>
{
    ...
    public ForgotPasswordEmailValidation(DbContext context)
    {
        ...
        RuleFor(x => x.EmailId).MustAsync(ValidateUser).WithErrorCode(AppVerseErrorRegistry.ErrorAuth101);
    ...
    {
        return await context.Set<AppVerseUser>().AnyAsync(u => u.Email == emailId, cancellation);
    }
}