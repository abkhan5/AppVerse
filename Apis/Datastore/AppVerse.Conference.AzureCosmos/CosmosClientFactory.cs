...
public class CosmosClientFactory
{
    ...
    //public async Task<CosmosClient> CreateClientAsync(bool allowBulkExecution,  CancellationToken cancellationToken)
    //{
    //    //if (!clientCache..ContainsKey(containerName))
    //    //if (!clientCache.TryGetValue(containerName, out _))
    ...
    //        {
    //            (cosmosDbOptions.Value.DatabaseName, containerName)
    //        };

    //        cosmosClient = await CosmosClient.CreateAndInitializeAsync(connectionString, containers, cosmosClientOptions: GetCosmosDbClientOptions(allowBulkExecution), cancellationToken: cancellationToken);
    //    }
    //    return cosmosClient;
    //}
